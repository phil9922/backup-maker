...
Where the config lives, how the daemon is put together, and how to build and
release it yourself. This page is for anyone editing config by hand or reading
the source — none of it is needed to use backup-maker.
...
[Building for the Pi](RASPBERRY-PI-SETUP.md#building-for-the-pi).
...
- [Security & safety properties](security.md) — what `state.json` holds and
  what is never written to a destination.
- [When a destination fills up](space.md) — the `min_free_gb` setting.
- [Monitoring your backups](monitoring.md) — the `[general]` settings the
  dashboard's Settings panel edits: `desktop_alerts` and the per-category
  `[general.alerts]` switches, `[general.webhook]` and `[general.ntfy]`
  delivery, and `lan_view` / `lan_view_access`. The addresses and tokens those
  last two delivery methods need live in `state.json`, not here.
