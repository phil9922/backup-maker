...
- **exFAT** works everywhere (Linux, Windows, macOS) and has no 4GB file
  limit — the safest choice for drives that may move between machines, and
  the one that lets you read your backups on a borrowed computer if the
  machine they came from is dead.
  - **Format it with a small cluster size**, e.g. `mkfs.exfat -c 4K`.
    Formatting tools pick a default from the drive's size — 128KB on a 128GB
    card — and every file, however small, occupies at least one cluster. A
    folder of source code we backed up held 1.2GB of data in **11GB** of
    space at the default; the same files took **1.4GB** at 4K. Versioning
    multiplies the waste, because each kept version rounds up too.
...
- [Choosing destinations](../guide/3-destinations.md) — the arrangement worth copying, and
  how to size it before you buy.
- [Setting up a Raspberry Pi as a backup target](raspberry-pi.md) — the
  always-on option, built step by step.
- [Sleeping computers](sleeping-computers.md) — why "always on" is the property
  that matters most in a destination.
