...
**Steps 1 to 7 below build route B**, because it puts nothing on the Pi beyond
Samba and is the shorter road to a working destination. Route A shares steps 1
to 4 exactly, and then
...
In the meantime, [Getting started](getting-started.md#adding-backup-targets)
covers adding a network drive from the CLI, and
[Choosing destinations](destinations.md) describes the three backup jobs.

## Building for the Pi

This matters if you'll run backup-maker on the Pi (route A), and it's why
steps 1 and 2 insist on a 64-bit image. Route B installs nothing on the Pi
beyond Samba.
...
This is route A — the paired-machine route. It replaces steps 5 and 7 above:
...
- [RECOMMENDED-HARDWARE.md](RECOMMENDED-HARDWARE.md) — choosing the SSD, and
  avoiding counterfeit media.
- [Sleeping computers](sleeping-computers.md) — the problem an always-on Pi
  makes impossible.
- [Monitoring your backups](monitoring.md) — the status page this Pi can serve,
  and the read-only network view on the machine being backed up.
- [Getting started](getting-started.md) — pointing your main machine at the
