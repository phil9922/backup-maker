...
func wizardAddArchive(cfg *config.Config, state *config.State, in *bufio.Reader, readLine func(string) string) error {
	...
	fmt.Println("A password is REQUIRED — archive contents are AES-256 encrypted.")
	fmt.Println("If you lose this password, the archives cannot be opened. Store it somewhere safe.")
	fmt.Println("It protects the contents, not the file list: like any zip, a snapshot lists")
	fmt.Println("its file names and sizes in the clear.")
