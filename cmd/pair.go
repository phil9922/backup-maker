...
var pairCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Printed last, so anything scripted around this command still reads the
		// same first four lines. A phone camera lifts the ID straight off the
		// terminal, which on a headless box over SSH beats transcribing 63
		// characters by hand. The ID is the contract; if the QR can't be built,
		// it simply isn't shown.
		if qr, qerr := qrcode.New(id, qrcode.Medium); qerr == nil {
			fmt.Println()
			fmt.Println("Or scan this with a phone:")
			// Rendered for a light-on-dark terminal, which is what a QR reader
			// needs: the code's dark modules are the terminal's background.
...
var pairAcceptCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		added, err := pairing.AcceptSource(id)
		if err != nil {
			return err
		}
		if !added {
