...
// The lifetime odometer takes StoredBytes, so it has to be exactly what sits on
// the destination — not the uncompressed source total, which is a different and
// larger number. Quoting the latter under a heading of "backed up in total"
// would claim more data was written than the destination ever received.
func TestRunReportsWhatLandedOnTheDestination(t *testing.T) {
	...
	fi, err := os.Stat(filepath.Join(dst, filepath.FromSlash(res.File)))
	if err != nil {
		t.Fatal(err)
	}
	if res.StoredBytes != fi.Size() {
		t.Errorf("StoredBytes = %d, but the snapshot on disk is %d bytes",
			res.StoredBytes, fi.Size())
	}
	if res.Bytes != 9 { // "alpha" + "beta"
		t.Errorf("source total = %d, want 9", res.Bytes)
	}
	if res.StoredBytes == res.Bytes {
		t.Error("StoredBytes is just the source total; the zip's real size was never measured")
	}
	...
	if failed.StoredBytes != 0 {
		t.Errorf("a failed run reported %d stored bytes", failed.StoredBytes)
	}
}
