...
func probe(t *testing.T, method, path string) (code int, reached bool) {
	t.Helper()
	h := lanReadOnlyOpen(everythingHandler(&reached), func() any { return map[string]any{} })
...
func TestNetworkStatusPublishesReceivedFoldersWithoutTheirPaths(t *testing.T) {
	...
	var reached bool
	h := lanReadOnlyOpen(everythingHandler(&reached), func() any { return m })
	req := httptest.NewRequest(http.MethodGet, "/api/status", nil)
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
...
func TestNetworkStatusNeedsNoToken(t *testing.T) {
	var reached bool
	h := lanReadOnlyOpen(everythingHandler(&reached), func() any {
...
func lanReadOnlyOpen(next http.Handler, status func() any) http.Handler {
	return lanReadOnly(next, status, nil)
}
