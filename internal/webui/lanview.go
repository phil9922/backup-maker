...
func RedactForNetwork(v any) any {
	...
	// the other direction: it quotes the destination's own error, which names the
	// share or the path it could not read the snapshot back from. The unverified
	// FLAG stays — "this snapshot was not checked" is a health fact, like
	// space_unknown, and somebody watching from another room is owed it.
