...
func (c *Client) do(method, path string, out any) error {
	...
	req.Header.Set(webui.MutationHeader, "1")
...
func (c *Client) post(path string, body any, out any) error {
	...
	// Required by the daemon on anything that changes something: it is what a
	// hostile page in a browser cannot send. See webui.MutationHeader.
	req.Header.Set(webui.MutationHeader, "1")
	resp, err := c.http.Do(req)
	if err != nil {
		return err
...
// RevertFolder asks the daemon to discard local changes to a backup this
// machine holds for another machine. Destructive: files that exist only here
// are deleted. A nil error means the engine accepted the request, NOT that the
// files are back yet — the copy takes as long as it takes.
func (c *Client) RevertFolder(folderID string) (string, error) {
	var out struct {
		Message string `json:"message"`
	}
