...
//
// One caveat is worth knowing about rather than hiding: applyConfig holds d.mu
// while it refreshes each destination's adoption manifest, which does touch the
// network. That hold is bounded by smbfs's own timeouts (5s to dial, 60s for an
// operation) per share target, so the threshold before pings stop — half the
// watchdog interval, 90 seconds with the unit's WatchdogSec=180, and systemd's
// own 180 on top of that — is what keeps a slow reload from ever being mistaken
// for a deadlock.
