...
func (d *daemon) applyConfig(ctx context.Context, cfg *config.Config) {
	...
	for _, t := range cfg.Targets {
		...
		if sb, _, _, berr := d.buildBackend(t); berr == nil {
			...
			// Best-effort: an unwritable destination just keeps its older copy
			// until the next apply reaches it.
			if err := setup.WriteManifest(sb, cfg, d.state.DriveTargetUUIDs); err != nil {
