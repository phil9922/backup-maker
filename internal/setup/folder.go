...
// semantics as the wizard's edit, where the box you are shown is the list you
// end up with. It saves config.toml; a running daemon picks the change up
// within seconds.
//
// Excluding something never deletes a backup: files that start matching stop
// being copied, and the copies already on a destination are versioned away
// rather than removed.
func SetFolderIgnores(id string, extraIgnore []string, noDefaultIgnores bool) error {
	cfg, err := config.Load()
	if err != nil {
		return err
	}
	...
	cfg.Folders[idx].ExtraIgnore = normalizeIgnores(extraIgnore)
	cfg.Folders[idx].NoDefaultIgnores = noDefaultIgnores
	return cfg.Save()
}

// normalizeIgnores tidies patterns typed by a human: surrounding whitespace is
// never meaningful in a pattern, and a repeated one is a no-op that would only
// clutter the file. Order is kept, since the list is read back to the user.
//
// Returns nil rather than an empty slice so "nothing excluded" leaves the key
// out of config.toml entirely (the field is toml `omitempty`).
func normalizeIgnores(patterns []string) []string {
	var out []string
