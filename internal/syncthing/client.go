...
// Ignores returns the ignore patterns currently stored for a folder.
func (c *Client) Ignores(folderID string) ([]string, error) {
	var out struct {
		Ignore []string `json:"ignore"`
	}
	err := c.do(http.MethodGet, "/rest/db/ignores?folder="+url.QueryEscape(folderID), nil, &out)
	return out.Ignore, err
}
