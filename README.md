...
Prefer to compile it? See [Building from source](#building-from-source).
...
watts, silent, and with no sleep mode to fall into — and
[docs/RASPBERRY-PI-SETUP.md](docs/RASPBERRY-PI-SETUP.md) walks through the
whole setup, from empty microSD card to receiving backups. Two ways to use
one:
...
![The wizard's first step with three choice cards: Incremental, Timed, and Restore this machine — I already have backups on a drive or network share](docs/screenshots/12-wizard-restore-choice.png)
...
![The adopt identity step asking: is this the same machine? Continue as "oldbox" — backups keep building exactly where they are — or start fresh under a new name](docs/screenshots/13-adopt-identity.png)
...
Not yet built: one-click revert for receive-side drift, Windows firewall
helper command, OS-keychain credential storage.
