...
Not yet built: QR-code pairing, one-click revert for receive-side drift,
Windows firewall helper command, OS-keychain credential storage. Known rough
edge: a single very large file leaves the progress bar static until it lands,
because progress is counted per completed file rather than per byte.
